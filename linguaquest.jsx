...
function LeagueShield({color="#CD7F32",size=48,tier=0}){
  const C=[
    ...
    {main:"#58D4F0",dark:"#2898C0",light:"#A0EFFF",glow:"#58D4F044",gem:"#70E0FF",gemLight:"#C8F4FF"},
  ...
  return <svg viewBox="0 0 120 155" width={size} height={size*1.29} style={{display:"block",overflow:"visible"}}>
    ...
    <g filter={`url(#${uid}s)`}>
      {/* ═══ Wide rounded shield body — Duolingo trophy shape ═══ */}
      {/* Outer shell — dark edge */}
      ...
      {/* Crown bumps at top — 3 rounded scallops */}
      ...
      {/* Facet lines — diamond cut pattern */}
      <polygon points="60,30 82,54 60,82 38,54" fill="#fff" opacity=".1"/>
      <polygon points="60,30 70,42 60,56 50,42" fill="#fff" opacity=".22"/>
      <polygon points="50,42 60,56 38,54" fill="#fff" opacity=".07"/>
      <polygon points="70,42 82,54 60,56" fill="#fff" opacity=".05"/>
      ...
      {/* Double shine highlight — makes it look glassy/3D */}
      <ellipse cx="48" cy="38" rx="14" ry="16" fill="#fff" opacity=".15" transform="rotate(-18 48 38)"/>
      <ellipse cx="46" cy="34" rx="6" ry="7" fill="#fff" opacity=".25" transform="rotate(-12 46 34)"/>
      {/* Outer edge highlight stroke */}
      <path d="M60 11 C70 9 84 14 94 22 Q102 30 104 44" fill="none" stroke="#fff" strokeWidth="1.8" opacity=".12" strokeLinecap="round"/>
      {/* ═══ Stem ═══ */}
      <path d="M50 108 L50 118 Q50 122 54 122 L66 122 Q70 122 70 118 L70 108" fill="#A8B0C0"/>
      <rect x="53" y="110" width="14" height="4" rx="2" fill="#C8D0D8" opacity=".5"/>
      {/* ═══ Two-tier pedestal base ═══ */}
      <path d="M36 122 L84 122 L88 130 Q88 134 84 134 L36 134 Q32 134 32 130 Z" fill={`url(#${uid}b)`}/>
      <rect x="38" y="124" width="44" height="3" rx="1.5" fill="#D8E0E8" opacity=".35"/>
      ...
      {/* Small center detail on lower base */}
...
function TapText({text,dict,style={},wordStyle={}}){
  ...
  return <span style={{...style,position:"relative"}}>
    {words.map((w,i)=>{
      ...
      const hasMeaning=clean.length>=2&&dict[clean];
      return <span key={i} onClick={hasMeaning?(e)=>handleTap(w,e):undefined}
    ...
    {tip&&<div style={{position:"fixed",left:tipPos.x,top:tipPos.y,transform:"translate(-50%,-100%)",
      padding:"8px 14px",borderRadius:12,background:"linear-gradient(135deg,#1e2a3a,#1a1a2e)",
      border:"1.5px solid rgba(28,176,246,.3)",boxShadow:"0 8px 24px rgba(0,0,0,.4)",
      zIndex:50,animation:"bounceIn .25s ease",pointerEvents:"none",whiteSpace:"nowrap",maxWidth:240}}>
      <div style={{fontSize:11,fontWeight:800,color:"#1CB0F6",fontFamily:"'Fredoka',sans-serif",marginBottom:2}}>{tip.word}</div>
      <div style={{fontSize:13,fontWeight:700,color:"#fff"}}>{tip.meaning}</div>
...
function MatchExercise({pairs,onComplete}){const[sel,setSel]=useState(null);const[matched,setMatched]=useState([]);const[wrongPair,setWrongPair]=useState(null);const busyRef=useRef(false);const[shuffled]=useState(()=>[...pairs.map(p=>p[1])].sort(()=>Math.random()-.5));const[,bump]=useState(0);
...
function SpeakExercise({phrase,meaning,langCode,onComplete}){
  ...
  return <div style={{textAlign:"center",animation:"fadeUp .35s ease"}}>
    ...
    <div style={{padding:"18px 20px",borderRadius:18,background:"linear-gradient(145deg,rgba(28,176,246,.06),rgba(28,176,246,.02))",border:"2px solid rgba(28,176,246,.12)",marginBottom:6,animation:"bounceIn .4s ease"}}>
      ...
      <p style={{fontSize:26,fontWeight:900,fontFamily:F,color:"#fff",lineHeight:1.4}}>{phrase}</p>
    </div>
...
export default function LinguaQuest(){
  ...
  if(lang?.id!==dictLangRef.current){
    ...
    units.forEach(u=>u.lessons?.forEach(l=>(l.ex||[]).forEach(e=>{
      if(e.t==="translate"&&e.p&&e.a)d[e.a.toLowerCase()]=e.p;
      if(e.t==="listen"&&e.w&&e.m){d[e.w.toLowerCase()]=e.m;e.w.split(/\s+/).forEach(w=>{const c=w.toLowerCase().replace(/[.,!?¿¡'"]/g,"");if(c.length>2)d[c]=e.m;});}
      if(e.t==="speak"&&e.ph&&e.m){d[e.ph.toLowerCase()]=e.m;e.ph.split(/\s+/).forEach(w=>{const c=w.toLowerCase().replace(/[.,!?¿¡'"]/g,"");if(c.length>2)d[c]=e.m;});}
      if(e.t==="match"&&e.pairs)e.pairs.forEach(([en,fo])=>{d[fo.toLowerCase()]=en;});
      if(e.t==="fill"&&e.a&&e.h)d[e.a.toLowerCase()]=e.h;
      if(e.t==="scramble"&&e.w&&e.h)d[e.w.toLowerCase()]=e.h;
    })));
    ...
    {screen==="lesson"&&lesson&&(<div style={{maxWidth:500,margin:"0 auto",padding:"0 20px",position:"relative",zIndex:2,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
      ...
      {!complete?(<div style={{flex:1,display:"flex",flexDirection:"column"}}><div key={exIdx} style={{flex:1,padding:"10px 0",animation:"fadeUp .3s ease"}}>
        ...
        {ex?.type==="translate"&&(<div><p style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,.35)",textAlign:"center",textTransform:"uppercase",letterSpacing:2,marginBottom:6,fontFamily:F}}>Translate this word</p>
          <p style={{fontSize:32,fontWeight:900,textAlign:"center",marginBottom:24,animation:"bounceIn .4s ease",fontFamily:F}}>{ex.prompt}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{ex.options.map((opt,i)=>{const isAns=normalize(opt)===normalize(ex.answer),isPicked=picked===opt;let bg="rgba(255,255,255,.04)",bd="rgba(255,255,255,.1)";if(result&&isAns){bg="rgba(88,204,2,.12)";bd="#58CC02";}else if(result==="wrong"&&isPicked){bg="rgba(255,75,75,.12)";bd="#FF4B4B";}
            return <button key={opt} disabled={!!result} onClick={()=>submitAnswer(opt,opt)} style={{position:"relative",padding:"14px 16px",borderRadius:14,border:`3px solid ${bd}`,backgroundColor:bg,color:"#fff",fontSize:15,fontWeight:700,cursor:result?"default":"pointer",transition:"all .2s",userSelect:"none",animation:`fadeUp .25s ease ${i*.06}s both`}}>
              {opt}{result&&isAns&&<span style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",animation:"bounceIn .3s ease"}}>✓</span>}
        ...
        {ex?.type==="listen"&&(<div><p style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,.35)",textAlign:"center",textTransform:"uppercase",letterSpacing:2,marginBottom:6,fontFamily:F}}>What does this mean?</p>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{ex.options.map((opt,i)=>{const isAns=normalize(opt)===normalize(ex.meaning),isPicked=picked===opt;let bg="rgba(255,255,255,.04)",bd="rgba(255,255,255,.1)";if(result&&isAns){bg="rgba(88,204,2,.12)";bd="#58CC02";}else if(result==="wrong"&&isPicked){bg="rgba(255,75,75,.12)";bd="#FF4B4B";}
            return <button key={opt} disabled={!!result} onClick={()=>submitAnswer(isAns?ex.meaning:"__wrong__",opt)} style={{padding:"14px 16px",borderRadius:14,border:`3px solid ${bd}`,backgroundColor:bg,color:"#fff",fontSize:14,fontWeight:700,cursor:result?"default":"pointer",transition:"all .2s",textAlign:"left",userSelect:"none",animation:`fadeUp .25s ease ${i*.06}s both`}}>
              {opt}{result&&isAns&&<span style={{float:"right",animation:"bounceIn .3s ease"}}>✓</span>}{result==="wrong"&&isPicked&&<span style={{float:"right",animation:"bounceIn .3s ease"}}>✗</span>}
            </button>;})}</div></div>)}
        ...
        {/* MATCH */}
        {ex?.type==="match"&&<MatchExercise key={`m-${exIdx}`} pairs={ex.pairs} onComplete={onMatchDone}/>}
        ...
        {ex?.type==="story"&&(<div>
          <div style={{padding:"16px",borderRadius:16,background:"linear-gradient(145deg,rgba(28,176,246,.06),rgba(28,176,246,.02))",border:"2px solid rgba(28,176,246,.1)",marginBottom:16,animation:"fadeUp .4s ease"}}>
            ...
            <p style={{fontSize:13,lineHeight:1.7,color:"rgba(255,255,255,.7)",fontWeight:600}}>{ex.story}</p></div>
          <p style={{fontSize:13,fontWeight:800,color:"rgba(255,255,255,.55)",textAlign:"center",marginBottom:12,fontFamily:F}}>{ex.question}</p>
        ...
        {ex?.type==="scramble"&&result&&(<div style={{textAlign:"center",animation:"fadeUp .3s ease"}}><p style={{fontSize:15,fontWeight:800,color:"rgba(255,255,255,.6)",marginBottom:6,fontFamily:F}}>🧩 Unscrambled!</p><p style={{fontSize:32,fontWeight:900,color:"#58CC02",fontFamily:F,animation:"bounceIn .4s ease"}}>{ex.word}</p></div>)}

        {/* SPEAK */}
        {ex?.type==="speak"&&!result&&<SpeakExercise key={`sp-${exIdx}`} phrase={ex.phrase} meaning={ex.meaning} langCode={SPEECH_LANGS[lang?.id]} onComplete={onSpeakDone}/>}
    ...
    {screen==="path"&&activeTab==="leagues"&&lang&&(()=>{
      ...
      return <div style={{maxWidth:480,margin:"0 auto",padding:"0 20px 90px",position:"relative",zIndex:2}}>
        ...
        <div style={{padding:"20px 0 16px",textAlign:"center",position:"relative",overflow:"hidden"}}>
          {/* Background glow */}
          <div style={{position:"absolute",top:"20%",left:"50%",transform:"translateX(-50%)",width:250,height:250,borderRadius:"50%",
            background:`radial-gradient(circle,${lg.color}25 0%,${lg.color}08 40%,transparent 70%)`,
            animation:"pulse 3s ease-in-out infinite",pointerEvents:"none"}}/>

          {/* Rotating light rays */}
          <div style={{position:"absolute",top:"30%",left:"50%",transform:"translateX(-50%)",width:200,height:200,animation:"leagueRays 20s linear infinite",opacity:.12,pointerEvents:"none"}}>
            {[...Array(8)].map((_,i)=><div key={i} style={{position:"absolute",top:"50%",left:"50%",width:2.5,height:100,
              background:`linear-gradient(180deg,${lg.color},transparent)`,transformOrigin:"top center",
              transform:`rotate(${i*45}deg)`,borderRadius:4}}/>)}
          </div>

          {/* Floating sparkle particles */}
          <div style={{position:"absolute",inset:0,pointerEvents:"none",overflow:"hidden"}}>
            {[...Array(12)].map((_,i)=><div key={i} style={{position:"absolute",
              width:3+Math.random()*5,height:3+Math.random()*5,borderRadius:"50%",
              background:i%3===0?"#fff":i%3===1?lg.color:"#FFC800",
              left:`${15+Math.random()*70}%`,top:`${10+Math.random()*50}%`,
              opacity:.15+Math.random()*.25,
              animation:`sparkle ${1.5+Math.random()*2}s ease-in-out ${Math.random()*2}s infinite`,
              boxShadow:`0 0 ${4+Math.random()*6}px ${i%2===0?lg.color:"#fff"}44`}}/>)}
          </div>

          {/* Shield badge — floating animation */}
          <div style={{position:"relative",zIndex:5,display:"inline-block",marginBottom:8}}>
            <div style={{animation:"charIdle 3s ease-in-out infinite",filter:`drop-shadow(0 0 20px ${lg.color}44) drop-shadow(0 4px 12px rgba(0,0,0,.3))`}}>
              <LeagueShield color={lg.color} tier={lg.tier} size={80}/>
            </div>
            {/* Small orbiting sparkles around shield */}
            {[0,120,240].map((deg,i)=><div key={i} style={{position:"absolute",top:"50%",left:"50%",width:6,height:6,borderRadius:"50%",
              background:i===0?"#fff":i===1?lg.color:"#FFC800",
              boxShadow:`0 0 8px ${i===0?"#fff":lg.color}66`,
              transform:`rotate(${deg}deg) translateX(50px) translateY(-50%)`,
              animation:`leagueRays ${4+i}s linear infinite`,opacity:.6}}/>)}
          </div>
      ...
      {leagueAnim.type==="promote"&&<>
        ...
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
          <div style={{position:"relative",marginBottom:40}}>
            ...
            {[...Array(8)].map((_,i)=><div key={i} style={{position:"absolute",width:4+Math.random()*6,height:4+Math.random()*6,borderRadius:"50%",
              background:leagueAnim.to.color,opacity:.2+Math.random()*.3,
              left:`${-30+Math.random()*240}px`,top:`${-30+Math.random()*280}px`,
              animation:`sparkle ${1.5+Math.random()*2}s ease-in-out ${Math.random()*2}s infinite`,
              boxShadow:`0 0 8px ${leagueAnim.to.color}44`}}/>)}

            {/* Trophy — uses LeagueShield at large size */}
            <div style={{animation:"leagueShieldIn .8s cubic-bezier(.4,2,.6,1) both",filter:`drop-shadow(0 8px 30px ${leagueAnim.to.color}33)`}}>
              <LeagueShield color={leagueAnim.to.color} tier={leagueAnim.to.tier} size={180}/>
            </div>
          </div>
          ...
          <div style={{textAlign:"center",animation:"leagueTextIn .6s ease .5s both"}}>
            ...
            <p style={{fontSize:15,color:"#8E8E8E",marginTop:14,fontWeight:700}}>
